...
int main () {
    ...
    while (running) {
        if (prompting) { // True in the first iteration, thereafter only if the old IO Task finished
            ioTask = std::async(std::launch::async, [&](){
                std::getline(std::cin,stdin);
            });
        }

        // Prepare starting a new IO Task in the next iteration if the old IO Task finished
        prompting = (ioTask.wait_for(std::chrono::seconds(0)) == std::future_status::ready);

        // If the old IO Task finished, a command has been received that needs to be handled
        if (prompting) {
            ...
            if (command == "uci") {
                ...
                searchTask = std::async(std::launch::async, [&board]() { return start_negamax(board, 5); });
                searching = true;
        ...
        if (searching && searchTask.valid() && searchTask.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
...
int pieceeval (Board& board) {
    ...
    eval += 3 * builtin::popcount(board.pieces(PieceType::KNIGHT, Color::WHITE) | board.pieces(PieceType::BISHOP, Color::WHITE));
    ...
    eval -= 3 * builtin::popcount(board.pieces(PieceType::KNIGHT, Color::BLACK) | board.pieces(PieceType::BISHOP, Color::BLACK));
